...
double ejecutarExperimento(int cantidadArchivo,  int threads) {
...
int main(int argc, char* argv[]) {
    ...
    for (int j = 12; j < 13; j++)
    {
        const string OUTPUT_FILE = "resultados/divir_archivos_thread_" + to_string(j) + "_3raVuelta.txt";
        ofstream outFile(OUTPUT_FILE);
        for (int i = 1; i < 16; i++){
            double duracion = ejecutarExperimento(i , j); 
