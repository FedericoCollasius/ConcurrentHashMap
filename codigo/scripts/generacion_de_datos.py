...
def guardar_archivo(palabras, nombre_archivo):
    ...
    with open(f"{directorio}/{nombre_archivo}", "w") as f:
        for palabra in palabras:
            f.write(palabra + '\n')

def generar_archivo_datos(num_palabras=1000, longitud_palabra=None, idioma=True, num_archivos=1):
    if idioma:
    ...
    for i in range(num_archivos):
        ...
        guardar_archivo(palabras_actuales, nombre_archivo)
        print(f"Archivo {nombre_archivo} generado")

# Ejemplo 
generar_archivo_datos(num_palabras=100, longitud_palabra=(3, 7), num_archivos=3)
